...
pub fn order_into_switches_set<E: Engine, CS: ConstraintSystem<E>>(
    ...
    for column_idx in 0..num_columns {
        ...
        for packet_idx in 0..size {
            if topology.topology[column_idx][packet_idx].0
                == topology.topology[column_idx][packet_idx].1
            {
                ...
                if routed_packages.contains(&routed_into_straght)
                    || routed_packages.contains(&routed_into_cross)
                {
                    continue;
                }
        ...
        let min = sorted_copy
            .drain(0..1)
            .next()
            .expect("must contain an element");
        let mut prev = min;
    ...
    for column_idx in 0..num_columns {
        ...
        for packet_idx in 0..topology.size {
                ...
                == topology.topology[column_idx][packet_idx].1
            {
                // straight switch, there is no need to allocate witness
                let routed_into_idx = topology.topology[column_idx][packet_idx].0;
                let previous_level_variable = permutation
                    .get(packet_idx)
                    .expect("must be a variable for this packet idx");
                // let previous_level_variable = previous_level_variable.ok_or(SynthesisError::Unsatisfiable)?.as_ref();
                let previous_level_variable =
                    previous_level_variable.ok_or(SynthesisError::Unsatisfiable)?;
                ...
                // may be we have already routed a pair, so quickly check
                if result_of_this_column[routed_into_straght].is_some()
                    || result_of_this_column[routed_into_cross].is_some()
                {
                    assert!(
                        result_of_this_column[routed_into_straght].is_some()
                            && result_of_this_column[routed_into_cross].is_some()
                    );
                    continue;
                }
                ...
                let previous_level_variable = permutation
                    ...
                    .as_ref()
                    .ok_or(SynthesisError::Unsatisfiable)?;
                let previous_level_pair = permutation
                    .get(another_idx)
                    .ok_or(SynthesisError::Unsatisfiable)?
                    .as_ref()
                    .ok_or(SynthesisError::Unsatisfiable)?;
    ...
    fn test_permutation_positive() {
        ...
        for size in 3..10 {
            ...
            for _ in 0..1 {
                ...
                for (_i, (o, p)) in original_vector
                    .into_iter()
                    .zip(permuted_vector.into_iter())
                    .enumerate()
                {
