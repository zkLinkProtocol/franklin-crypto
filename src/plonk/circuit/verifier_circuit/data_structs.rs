...
use crate::bellman::plonk::better_better_cs::cs::{
    ConstraintSystem, PlonkConstraintSystemParams, Variable,
};
...
impl<'a, E: Engine, WP: WrappedAffinePoint<'a, E>> ProofGadget<'a, E, WP> {
    pub fn alloc<CS: ConstraintSystem<E>, P: OldCSParams<E>, AD: AuxData<E>>(
        ...
        let grand_product_commitment =
            WrappedAffinePoint::alloc(cs, Some(proof.grand_product_commitment), params, aux_data)?;

        let quotient_poly_commitments = proof
            .quotient_poly_commitments
            .iter()
            .map(|x| WrappedAffinePoint::alloc(cs, Some(*x), params, aux_data))
            .collect::<Result<Vec<_>, _>>()?;

        let wire_values_at_z = proof
            .wire_values_at_z
            .iter()
            .map(|x| AllocatedNum::alloc(cs, || Ok(*x)))
            .collect::<Result<Vec<_>, _>>()?;

        let wire_values_at_z_omega = proof
        ...
        let grand_product_at_z_omega =
            AllocatedNum::alloc(cs, || Ok(proof.grand_product_at_z_omega))?;
        let quotient_polynomial_at_z =
            AllocatedNum::alloc(cs, || Ok(proof.quotient_polynomial_at_z))?;
        let linearization_polynomial_at_z =
            AllocatedNum::alloc(cs, || Ok(proof.linearization_polynomial_at_z))?;

        let permutation_polynomials_at_z = proof
            .permutation_polynomials_at_z
            .iter()
            .map(|x| AllocatedNum::alloc(cs, || Ok(*x)))
            .collect::<Result<Vec<_>, _>>()?;

        let opening_at_z_proof =
            WrappedAffinePoint::alloc(cs, Some(proof.opening_at_z_proof), params, aux_data)?;
        let opening_at_z_omega_proof =
            WrappedAffinePoint::alloc(cs, Some(proof.opening_at_z_omega_proof), params, aux_data)?;
    ...
    pub fn alloc_from_witness<CS: ConstraintSystem<E>, P: OldCSParams<E>, AD: AuxData<E>>(
        ...
        for idx in 0..num_inputs {
            let wit = proof
                .as_ref()
                .and_then(|el| Some(&el.input_values))
        ...
        for idx in 0..state_width {
            let wit = proof
                .as_ref()
                .and_then(|el| Some(&el.wire_commitments))
                .and_then(|el| Some(el[idx]));
        ...
        let wit = proof
            .as_ref()
            .and_then(|el| Some(el.grand_product_commitment));
        ...
        for idx in 0..num_quotient_commitments {
            let wit = proof
                .as_ref()
                .and_then(|el| Some(&el.quotient_poly_commitments))
                .and_then(|el| Some(el[idx]));
            let allocated = WrappedAffinePoint::alloc(cs, wit, params, aux_data)?;
        ...
        for idx in 0..state_width {
            let wit = proof
                .as_ref()
                .and_then(|el| Some(&el.wire_values_at_z))
                .and_then(|el| Some(el[idx]));
            let allocated = AllocatedNum::alloc(cs, || Ok(*wit.get()?))?;
        ...
        for idx in 0..1 {
            let wit = proof
                .as_ref()
                .and_then(|el| Some(&el.wire_values_at_z_omega))
                .and_then(|el| Some(el[idx]));
            let allocated = AllocatedNum::alloc(cs, || Ok(*wit.get()?))?;
        ...
        let wit = proof
            .as_ref()
            .and_then(|el| Some(el.grand_product_at_z_omega));
        let grand_product_at_z_omega = AllocatedNum::alloc(cs, || Ok(*wit.get()?))?;

        let wit = proof
            .as_ref()
            .and_then(|el| Some(el.quotient_polynomial_at_z));
        let quotient_polynomial_at_z = AllocatedNum::alloc(cs, || Ok(*wit.get()?))?;

        let wit = proof
            .as_ref()
            .and_then(|el| Some(el.linearization_polynomial_at_z));
        ...
        for idx in 0..(state_width - 1) {
            let wit = proof
                .as_ref()
                .and_then(|el| Some(&el.permutation_polynomials_at_z))
                .and_then(|el| Some(el[idx]));
            let allocated = AllocatedNum::alloc(cs, || Ok(*wit.get()?))?;
        ...
        let wit = proof
            .as_ref()
            .and_then(|el| Some(el.opening_at_z_omega_proof));
...
impl<'a, E: Engine, WP: WrappedAffinePoint<'a, E>> VerificationKeyGagdet<'a, E, WP> {
    pub fn alloc<CS: ConstraintSystem<E>, P: OldCSParams<E>, AD: AuxData<E>>(
        ...
        let next_step_selector_commitments = vk
    ...
    for ProofGadget<'a, E, WP>
{
    fn into_witness<CS: ConstraintSystem<E>>(
        ...
        add_points(
            &[
                self.opening_at_z_proof.clone(),
                self.opening_at_z_omega_proof.clone(),
            ],
            &mut result,
        );
...
pub fn add_prime_field_elements<'a, E: Engine, F: PrimeField>(
    src: &[FieldElement<'a, E, F>],
    dst: &mut Vec<Num<E>>,
) {
    for el in src.iter() {
...
pub fn add_points<'a, E: Engine, WP: WrappedAffinePoint<'a, E>>(src: &[WP], dst: &mut Vec<Num<E>>) {
    for el in src.iter() {
        let p = el.get_point();
        let x = p.x.clone();
        let y = p.y.clone();
