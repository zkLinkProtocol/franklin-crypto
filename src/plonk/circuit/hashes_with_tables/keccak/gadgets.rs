...
impl<E: Engine> Keccak256Gadget<E> {
    pub fn new<CS: ConstraintSystem<E>>(
        ...
        let binary_base_num_of_chunks =
            binary_base_num_of_chunks.unwrap_or(DEFAULT_BINARY_NUM_OF_CHUNKS);
        let first_base_num_of_chunks =
            first_base_num_of_chunks.unwrap_or(DEFAULT_FIRST_BASE_NUM_OF_CHUNKS);
        let second_base_num_of_chunks =
            second_base_num_of_chunks.unwrap_or(DEFAULT_SECOND_BASE_NUM_OF_CHUNKS);
        ...
        for max_offset in offsets.iter().flat_map(|r| r.iter()) {
            ...
            while cur_offset < KECCAK_LANE_WIDTH {
                let step =
                    Self::check_offset_helper(first_base_num_of_chunks, cur_offset, *max_offset);
                assert!(step > 0);
        ...
        let from_binary_converter_table = get_or_create_table(cs, name1, || {
        ...
        let first_to_second_base_converter_table = get_or_create_table(cs, name2, || {
            LookupTableApplication::new(
                name2,
        ...
        let f = |x| keccak_u64_first_converter(x);
        let of_first_to_second_base_converter_table = get_or_create_table(cs, name3, || {
            LookupTableApplication::new(
                name3,
                OverflowCognizantConverterTable::new(
                    KECCAK_FIRST_SPARSE_BASE,
                    KECCAK_SECOND_SPARSE_BASE,
                    KECCAK_LANE_WIDTH as u64,
                    f,
                    name3,
                ),
                columns3.clone(),
        ...
        let from_second_base_converter_table = get_or_create_table(cs, name4, || {
            LookupTableApplication::new(
                name4,
                MultiBaseNormalizationTable::new(
        ...
        let range_table = match use_global_range_table {
            ...
            false => {
                let range_table = LookupTableApplication::new_range_table_of_width_3(
                    range_table_bitlen,
                    columns3,
                )?;
        ...
        let round_cnsts_in_first_base = [
            f(0x0000000000000001, t),
            f(0x0000000000008082, t),
            f(0x800000000000808A, t),
            f(0x8000000080008000, t),
            f(0x000000000000808B, t),
            f(0x0000000080000001, t),
            f(0x8000000080008081, t),
            f(0x8000000000008009, t),
            f(0x000000000000008A, t),
            f(0x0000000000000088, t),
            f(0x0000000080008009, t),
            f(0x000000008000000A, t),
            f(0x000000008000808B, t),
            f(0x800000000000008B, t),
            f(0x8000000000008089, t),
            f(0x8000000000008003, t),
            f(0x8000000000008002, t),
            f(0x8000000000000080, t),
            f(0x000000000000800A, t),
            f(0x800000008000000A, t),
            f(0x8000000080008081, t),
            f(0x8000000000008080, t),
            f(0x0000000080000001, t),
            f(0x8000000080008008, t),
        ];

        let r = KECCAK_SECOND_SPARSE_BASE;
        let round_cnsts_in_second_base = [
            f(0x0000000000000001, r),
            f(0x0000000000008082, r),
            f(0x800000000000808A, r),
            f(0x8000000080008000, r),
            f(0x000000000000808B, r),
            f(0x0000000080000001, r),
            f(0x8000000080008081, r),
            f(0x8000000000008009, r),
            f(0x000000000000008A, r),
            f(0x0000000000000088, r),
            f(0x0000000080008009, r),
            f(0x000000008000000A, r),
            f(0x000000008000808B, r),
            f(0x800000000000008B, r),
            f(0x8000000000008089, r),
            f(0x8000000000008003, r),
            f(0x8000000000008002, r),
            f(0x8000000000000080, r),
            f(0x000000000000800A, r),
            f(0x800000008000000A, r),
            f(0x8000000080008081, r),
            f(0x8000000000008080, r),
            f(0x0000000080000001, r),
            f(0x8000000080008008, r),
        ];
    ...
    fn cnst_rotate_and_convert(&self, fr: &E::Fr, rot: usize) -> E::Fr {
        ...
        for (is_first, is_last, _i) in (0..(KECCAK_LANE_WIDTH + 1)).identify_first_last() {
            let remainder = (input.clone() % BigUint::from(KECCAK_FIRST_SPARSE_BASE))
                .to_u64()
                .unwrap();
            if is_first || is_last {
    ...
    fn check_offset_helper(
        base_num_of_chunks: usize,
        cur_offset: usize,
        max_offset: usize,
    ) -> usize {
    ...
    pub fn convert_binary_to_sparse_repr<CS>(
        &self,
        cs: &mut CS,
        input: &Num<E>,
        sparse_base: KeccakBase,
    ) -> Result<Num<E>>
    where
            ...
            Num::Variable(var) => {
                ...
                let witnesses = match var.get_value() {
                    ...
                    Some(f) => {
                        ...
                        for _ in 0..num_slices {
                            let remainder = (big_f.clone() % BigUint::from(input_slice_modulus))
                                .to_u64()
                                .unwrap();
                ...
                for (_is_first, is_last, input_chunk) in input_slices.iter().identify_first_last() {
                    let (output1, output2, new_acc) = self.query_table_accumulate(
                        cs,
                        &self.from_binary_converter_table,
                        input_chunk,
                        &acc,
                        &coef,
                        is_last,
                ...
                AllocatedNum::long_weighted_sum_eq(
                    cs,
                    &output_slices[..],
                    &output_slice_modulus_fr,
                    &output_total,
                    false,
                )?;
    ...
    fn theta<CS: ConstraintSystem<E>>(
        &self,
        cs: &mut CS,
        state: KeccakState<E>,
        ...
        let coeffs = [
            E::Fr::one(),
            E::Fr::one(),
            u64_to_ff(KECCAK_FIRST_SPARSE_BASE),
        ];
    ...
    fn rho<CS: ConstraintSystem<E>>(
        ...
        let input_chunks_standard_step = u64_exp_to_ff(
            KECCAK_FIRST_SPARSE_BASE,
            self.first_base_num_of_chunks as u64,
        );
        let output_chunks_standard_step = u64_exp_to_ff(
            KECCAK_SECOND_SPARSE_BASE,
            self.first_base_num_of_chunks as u64,
        );
        ...
        for (i, j) in (0..KECCAK_STATE_WIDTH).cartesian_product(0..KECCAK_STATE_WIDTH) {
            ...
            let mut cur_output_coef: E::Fr = u64_exp_to_ff(
                KECCAK_SECOND_SPARSE_BASE,
                (KECCAK_LANE_WIDTH - offset) as u64,
            );
            let mut acc = var;
            ...
            let last_chunk_low_value = if has_value {
                ...
                let remainder = (raw_value.clone() % BigUint::from(divider))
                    .to_u64()
                    .unwrap();
                let new_val = u64_to_ff(remainder);
                raw_value /= divider;
            ...
            while cur_offset < KECCAK_LANE_WIDTH {
                let chunk_count_bound =
                    Self::check_offset_helper(self.first_base_num_of_chunks, cur_offset, offset);
                assert!(
                    (chunk_count_bound > 0) && (chunk_count_bound <= self.first_base_num_of_chunks)
                );
                ...
                if chunk_count_bound == self.first_base_num_of_chunks {
                    ...
                    let coef_step =
                        u64_exp_to_ff(KECCAK_FIRST_SPARSE_BASE, chunk_count_bound as u64);
                ...
                if cur_offset == offset {
                    ...
                    if chunk_count_bound == self.first_base_num_of_chunks {
                        ...
                        let coef_step =
                            u64_exp_to_ff(KECCAK_SECOND_SPARSE_BASE, chunk_count_bound as u64);
    ...
    fn xi_i<CS: ConstraintSystem<E>>(
        ...
        let output1_slice_modulus_fr =
            u64_exp_to_ff(KECCAK_FIRST_SPARSE_BASE, num_of_chunks as u64);
        ...
        let keccak_ff_second_converter = |fr: E::Fr, output_base: u64| {
            func_normalizer(fr, KECCAK_SECOND_SPARSE_BASE, output_base, |n| {
                keccak_u64_second_converter(n)
            })
        };
        ...
        for (j, i) in (0..KECCAK_STATE_WIDTH).cartesian_product(0..KECCAK_STATE_WIDTH) {
            ...
            let d = match elems_to_mix {
                None => {
                    if i == 0 && j == 0 {
                        Num::Constant(self.round_cnsts_in_second_base[round].clone())
                    } else {
                        Num::default()
                    }
                }
                Some(input_to_mix) => {
                    ...
                    if idx < KECCAK_RATE_WORDS_SIZE {
                        input_to_mix[idx].clone()
                    } else {
                        Num::default()
                    }
                }
            ...
            if lc.is_constant() {
                ...
                new_state[(i, j)] =
                    Num::Constant(keccak_ff_second_converter(fr, KECCAK_FIRST_SPARSE_BASE));
            ...
            for (_is_first, is_last, input_chunk) in input_slices.iter().identify_first_last() {
                let (output1, output2, new_acc) = self.query_table_accumulate(
                    cs,
                    &self.from_second_base_converter_table,
                    input_chunk,
                    &acc,
                    &coef,
                    is_last,
    ...
    pub fn keccak_f<CS: ConstraintSystem<E>>(
        ...
        let (mut new_state, out) = self.xi_i(
            cs,
            state,
            KECCAK_NUM_ROUNDS - 1,
            elems_to_squeeze,
        ...
        if elems_to_mix.is_some() && is_fixed_length {
            new_state[(0, 0)] = new_state[(0, 0)].add(
                cs,
                &Num::Constant(self.round_cnsts_in_first_base[KECCAK_NUM_ROUNDS - 1]),
            )?;
        }

        let squeezed = if elems_to_squeeze > 0 {
            Some(out)
        } else {
            None
        };
    ...
    pub fn digest<CS: ConstraintSystem<E>>(
        ...
        for (is_first, _is_last, data_block) in
            data.chunks(KECCAK_RATE_WORDS_SIZE).identify_first_last()
        {
            if is_first {
                for (idx, elem) in data_block.iter().enumerate() {
                    let out = self.convert_binary_to_sparse_repr(
                        cs,
                        elem,
                ...
                let converted: Vec<Num<E>> = data_block
                    .iter()
                    .map(|elem| {
                        self.convert_binary_to_sparse_repr(
                            cs,
                            elem,
                            KeccakBase::KeccakSecondSparseBase,
                        )
                    })
                    .collect::<Result<Vec<_>>>()?;
                let (new_state, _) =
                    self.keccak_f(cs, state, 0, Some(&converted[..]), false, true)?;
                state = new_state;
            }
        }

        while res.len() < self.digest_size {
            let elems_to_squeeze =
                std::cmp::min(self.digest_size - res.len(), KECCAK_RATE_WORDS_SIZE);
            let is_final = res.len() + KECCAK_RATE_WORDS_SIZE >= self.digest_size;

            let (new_state, squeezed) =
                self.keccak_f(cs, state, elems_to_squeeze, None, is_final, true)?;
            state = new_state;
    ...
    pub fn keccak_round_function_init<CS: ConstraintSystem<E>>(
        ...
        for (idx, elem) in initial_absorbed_values.iter().enumerate() {
            let out =
                self.convert_binary_to_sparse_repr(cs, elem, KeccakBase::KeccakFirstSparseBase)?;
            state[(idx % KECCAK_STATE_WIDTH, idx / KECCAK_STATE_WIDTH)] = out;
        }
    ...
    pub fn keccak_round_function<CS: ConstraintSystem<E>>(
        ...
        for (elem_in, elem_out) in elems_to_mix.iter().zip(elems_to_absorb.iter_mut()) {
            let tmp = if is_first {
                is_first = false;
                let tmp = self.convert_binary_to_sparse_repr(
                    cs,
                    &elem_in,
                    KeccakBase::KeccakSecondSparseBase,
                )?;
        ...
        let (rolled_state, squeezed_wrapped) = self.keccak_f(
            cs,
            state,
            DEFAULT_KECCAK_DIGEST_WORDS_SIZE,
            Some(&elems_to_absorb[..]),
            false,
            false,
        )?;
        ...
        for idx in 0..KECCAK_STATE_WIDTH * KECCAK_STATE_WIDTH {
            let rolled_state_elem =
                rolled_state[(idx % KECCAK_STATE_WIDTH, idx / KECCAK_STATE_WIDTH)].clone();
    ...
    pub fn digest_from_bytes<CS: ConstraintSystem<E>>(
        &self,
        cs: &mut CS,
        bytes: &[Byte<E>],
    ) -> Result<Vec<Num<E>>> {
