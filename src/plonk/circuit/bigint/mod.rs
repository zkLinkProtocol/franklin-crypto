...
pub fn split_some_into_slices<F: PrimeField>(
    ...
    if let Some(v) = el.as_ref() {
        split_into_slices(v, slice_width, num_slices)
            .into_iter()
            .map(|el| Some(el))
            .collect()
    } else {
...
pub fn create_range_constraint_chain<E: Engine, CS: ConstraintSystem<E>>(
    ...
    assert!(num_bits & 1 == 0);
    assert_eq!(
        CS::Params::STATE_WIDTH,
        4,
        "this only works for a state of width 4 for now"
    );
    ...
    let mut slices: Vec<Option<E::Fr>> = if let Some(v) = to_constraint.get_value() {
        split_into_bit_constraint_slices(&v, 2, num_elements)
            .into_iter()
            .map(|el| Some(el))
            .collect()
    } else {
...
const RANGE_CHECK_SINGLE_APPLICATION_TABLE_NAME: &'static str =
    "Range check table for a single column";

pub fn get_range_constraint_info<E: Engine, CS: ConstraintSystem<E>>(
    cs: &CS,
) -> Vec<RangeConstraintInfo> {
...
mod test {
    ...
    fn check_two_bit_gate() {
        ...
        impl Circuit<Bn256> for Tester {
            ...
            fn synthesize<CS: ConstraintSystem<Bn256>>(
                ...
                let variables: Vec<_> = (0..5)
                    .map(|_| AllocatedNum::alloc(cs, || Ok(Fr::one())).unwrap())
                    .collect();
        ...
        let crs_mons = Crs::<Bn256, CrsForMonomialForm>::crs_42(
            setup.gate_selectors_monomials[0].size(),
            &worker,
        );
