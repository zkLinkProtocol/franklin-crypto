// new test paradigm: using better_better_cs for witness generation and better_better_cs for actual constraint system
...
use crate::bellman::plonk::better_better_cs::cs::{
    ensure_in_map_or_create, get_from_map_unchecked, ArithmeticTerm, Circuit, Gate, GateInternal,
    MainGate, MainGateTerm, PlonkCsWidth4WithNextStepParams, PolyIdentifier, TrivialAssembly,
    Width4MainGateWithDNext,
};
use crate::bellman::plonk::better_better_cs::cs::{ConstraintSystem, Variable};
use crate::bellman::plonk::better_better_cs::data_structures::{
...
use crate::bellman::plonk::polynomials::{Coefficients, Polynomial, Values};
...
mod test {
        ...
        WP: WrappedAffinePoint<'a, E>,
    {
        ...
        print_verification_key(&verification_key);
        print_proof(&proof);
        dbg!(valid);
        println!("PROOF IS VALID");

        // let buffer = File::create("./new_proof.proof").unwrap();
        // let proof = Proof::<E,TestCircuit4<E>>::read(&buffer).unwrap();
        // let buffer = File::create("./verify_key.key").unwrap();
    ...
    fn bn256_recursion_test() {
        ...
        >(&rescue_params, &rns_params, transcript_params);
    }
}
