...
pub(crate) fn mmul_assign<E: Engine, const DIM: usize>(
    ...
    for col in 0..DIM {
        result[col] = crate::poseidon::common::utils::scalar_product::<E>(vector, &matrix[col]);
    }
...
pub(crate) fn multiply<E: Engine, const DIM: usize>(
    ...
    for (i, rv) in m1.iter().enumerate() {
        for (j, cv) in transposed_m2.iter().enumerate() {
            result[i][j] = crate::poseidon::common::utils::scalar_product::<E>(rv, cv);
        }
    }
...
mod test {
    use super::*;
    use crate::bellman::bn256::{Bn256, Fr};
    use crate::bellman::PrimeField;
    ...
    fn test_optimized_matrixes() {
        ...
        let original_mds = crate::poseidon::common::utils::construct_mds_matrix::<Bn256, _, DIM>(rng);
